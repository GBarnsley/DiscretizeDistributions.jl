@testset "discretise function tests with interval backend" begin
    ...
    @testset "Continuous distributions with fixed interval" begin
        ...
        @test eltype(support(discretised)) <: Interval
        ...
        @test eltype(support(discretised_custom)) <: Interval
        ...
        @test eltype(support(discretised_exp)) <: Interval
        @test sum(probs(discretised_exp)) ≈ 1.0 atol=1e-10
        
        # Check first interval starts at or near 0
        first_interval = minimum(support(discretised_exp))
        ...
        @test eltype(support(discretised_uniform)) <: Interval
        @test sum(probs(discretised_uniform)) ≈ 1.0 atol=1e-10
        
        # Check interval bounds cover the distribution support
        first_interval = minimum(support(discretised_uniform))
        last_interval = maximum(support(discretised_uniform))
    ...
    @testset "Continuous distributions with custom intervals" begin
        ...
        @test eltype(support(discretised_unsorted)) <: Interval
    ...
    @testset "Discrete distributions with fixed interval" begin
        ...
        # Check first interval starts at or near 0
        first_interval = minimum(support(discretised))
        @test inf(first_interval) >= -1e-10
        ...
        @test eltype(support(discretised_binomial)) <: Interval
        @test sum(probs(discretised_binomial)) ≈ 1.0 atol=1e-10
        
        # Check interval bounds are reasonable for bounded distribution
        first_interval = minimum(support(discretised_binomial))
        last_interval = maximum(support(discretised_binomial))
        @test inf(first_interval) >= -1e-10
        @test sup(last_interval) <= 10.0 + 1e-10
        ...
        @test eltype(support(discretised_geom)) <: Interval
        @test sum(probs(discretised_geom)) ≈ 1.0 atol=1e-10
        
        # Check first interval starts at or near 0
        first_interval = minimum(support(discretised_geom))
        @test inf(first_interval) >= -1e-10
    end
    ...
    @testset "Edge cases and error conditions" begin
        ...
        @test eltype(support(discretised_small)) <: Interval
        ...
        @test eltype(support(discretised_large)) <: Interval
        ...
        @test eltype(support(discretised_single)) <: Interval
        @test length(support(discretised_single)) >= 1  # Should have at least one interval
        ...
        @test eltype(support(discretised_extreme)) <: Interval
