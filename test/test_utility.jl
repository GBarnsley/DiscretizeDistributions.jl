@testset "Utility function tests for interval-based backend" begin
    ...
    @testset "remove_infinities function tests" begin
        ...
        discrete_with_extremes = discretize(
            normal_dist, 0.5; min_quantile = 1e-10, max_quantile = 1 - 1e-10)
        ...
        @test all(
            interval -> !isinf(inf(interval)) && !isinf(sup(interval)), cleaned_intervals)
        ...
        finite_dist = discretize(
            Normal(0, 1), 0.5; min_quantile = 0.01, max_quantile = 0.99)
    ...
    @testset "left_align_distribution function tests" begin
        ...
        finite_intervals = [interval
                            for interval in support(interval_dist)
                            if isfinite(inf(interval)) && isfinite(sup(interval))]
    ...
    @testset "centred_distribution function tests" begin
        ...
        finite_intervals = [interval
                            for interval in support(interval_dist)
                            if isfinite(inf(interval)) && isfinite(sup(interval))]
        expected_support = [(inf(interval) + sup(interval)) / 2
        ...
        expected_uniform = [(inf(interval) + sup(interval)) / 2
                            for interval in uniform_test_intervals]
    ...
    @testset "Edge cases and error conditions" begin
        ...
        extreme_dist = discretize(
            Normal(0, 1), 0.5; min_quantile = 1e-10, max_quantile = 1 - 1e-10)
        ...
        tiny_dist = discretize(
            Normal(0, 1), 0.001; min_quantile = 0.01, max_quantile = 0.99)
