@testset "Method parameter tests" begin
    ...
    # @testset "Unbiased method with equal intervals" begin
    #     # TODO: Fix unbiased method implementation
    #     # This is a complex feature that needs more work on the mathematical implementation
    # end
    ...
    @testset "Discrete distributions with alignment methods" begin
        ...
        @test all([result isa DiscreteNonParametric for result in [interval_result, left_result, centered_result, right_result]])
        @test eltype(support(interval_result)) <: Interval
        @test all([eltype(support(result)) <: Real for result in [left_result, centered_result, right_result]])
    ...
    @testset "Trapezoid points parameter tests" begin
        ...
        for tp in trapezoid_vals
            result = discretize(exp_dist, interval; method=:unbiased, trapezoid_points=tp)
            push!(results, result)
        ...
        for m in means
            @test abs(m - true_mean) < 0.02  # Within 2% error
        end
        ...
        @testset "Trapezoid points edge cases" begin
            normal_dist = Normal(1, 0.5)

            # Test minimum viable trapezoid_points
            result_min = discretize(normal_dist, 0.2; method=:unbiased, trapezoid_points=3)
            ...
            result_max = discretize(normal_dist, 0.2; method=:unbiased, trapezoid_points=100000)
            ...
            true_mean = mean(normal_dist)
            @test abs(mean(result_min) - true_mean) < 0.05
        ...
        @testset "Trapezoid points method specificity" begin
            ...
            for method in [:left_aligned, :centred, :right_aligned]  # Skip :interval due to interval comparison issues
                result_low = discretize(normal_dist, interval; method=method, trapezoid_points=100)
                result_high = discretize(normal_dist, interval; method=method, trapezoid_points=10000)
            ...
            interval_low = discretize(normal_dist, interval; method=:interval, trapezoid_points=100)
            interval_high = discretize(normal_dist, interval; method=:interval, trapezoid_points=10000)
            ...
            unbiased_low = discretize(normal_dist, interval; method=:unbiased, trapezoid_points=100)
            unbiased_high = discretize(normal_dist, interval; method=:unbiased, trapezoid_points=10000)
